...
### If you only need part of a date

If you only need some parts of a date, such as the day and month a financial
year ends on, set the `parts` prop to the parts you need. They are always shown
in date order. The default hint gives an example of those parts alone.

<Canvas of={Stories.DayAndMonth} />
