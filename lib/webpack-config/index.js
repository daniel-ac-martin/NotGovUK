...
const createDocsCompiler = require('@storybook/addon-docs/mdx-compiler-plugin');
...
const reactDocgenTypescriptLoader = require.resolve("react-docgen-typescript-loader");
  ...
  return {
    ...
    externals: (
      serverMode
        ? [
          nodeExternals({
            whitelist: undefined && [/\.(?!(?:[cm]?js|json)$).{1,5}$/i, /^@not-govuk[\\/](?!(?:.+[\\/]node_modules[\\/]))/]
          }),
          webpackConfExternal,
          entrypointsExternal
        ] : undefined
        ...
        cacheGroups: {
          ...
          vendors: {
            ...
            name: (module, chunks, cacheGroupKey) => {
              const packageName = module.context.match(/[\\/]node_modules[\\/](\.pnpm[\\/])?(registry\.npmjs\.org[\\/])?(.*?)([\\/]|$)/)[3];
    ...
    devtool: devMode ? 'eval-source-map' : 'inline-source-map',
    devServer: {
