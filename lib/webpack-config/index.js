...
import babelCorePkg from '@babel/core/package.json' with { type: 'json' };
import babelLoaderPkg from 'babel-loader/package.json' with { type: 'json' };

const version = pkg.version;
const babelVersion = babelCorePkg.version;
const babelLoaderVersion = babelLoaderPkg.version;
...
const babelPluginExportNS = require.resolve('@babel/plugin-transform-export-namespace-from');
const babelPluginClassProperties = require.resolve('@babel/plugin-transform-class-properties');
const babelPluginPrivateMethods = require.resolve('@babel/plugin-transform-private-methods');
...
const storybookLoader = require.resolve('@storybook/addon-docs/mdx-loader');
...
const generateConfig = (options) => {
  ...
  const tsConfig = resolve(options.baseDir, options.tsConfig);
  ...
  return {
    ...
    context: resolve(options.baseDir),
    entry: (
      serverMode
        ? {
    ...
    output: {
      ...
      path: resolve(options.baseDir, options.outDir),
      publicPath: '/public/',
    ...
    module: {
      rules: [
        ...
        },
        {
          ...
          type: 'asset/resource',
          generator: {
            filename: hashInName ? 'images/[name].[contenthash:8][ext]' : 'images/[name].[contenthash:6][ext]'
          }
        }
      ].filter(id)
    },
    ...
    plugins: [
      ...
      serverMode && new ForkTsCheckerWebpackPlugin({ typescript: {
        configFile: tsConfig,
